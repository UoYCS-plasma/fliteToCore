...
> compRhs :: Env -> Exp -> Compile ()
> compRhs env e = comp env 1 True e
>   where
>   fvs = freeVars e
>
...
>                 >> emit "L_PUSH(makeCTR(res));"
>        block [ "goto EVAL_RET;" | tail ]
>   comp env n tail (Force x e) =
>     do comp env 0 False (Var x)
>        comp ((x, Stack 1) : incStack 1 env) (n+1) tail e
...
>          Var v ->
>            do block [ "atom = " ++ offset (env!v) ++ ";" ]
>               case sum [1 | w <- fvs, w == v] == 1 of
>                 True  -> emit (offset (env!v) ++ " = makeINT(0);")
>                 False -> return ()
>               eval
>          other -> comp env 0 False e
>        emit "switch (res) {"
...
>     val (PRSApp f [x, y]) = prsEval env f x y
>     op "(+)"    = "+"
>     op "(-)"    = "-"
>     op "(==)"   = "=="
>     op "(/=)"   = "!="
...
> compileDecl (Func f [] e) | f /= "main" =
>   error ("0-arity funs not handled: " ++ f)
> compileDecl (Func f ps e) =
>   emit (funName f ++ ":") >>  -- emit ("printf(\"" ++ f ++ "\\n\");") >> 
>     heapCheck >> compRhs env e
>   where
>     vs = map (\(Var v) -> v) ps
...
>     , "prog_start = ((double) tv.tv_sec * 1000000) + (double) tv.tv_usec;"
...
> start = 
>   do emit "atom = makePTR(hp);"
>      emit "*hp++ = makeHDR(1); *hp++ =  makeFUN(&&F_main);"
>      emit "L_PUSH(atom);"
>      eval
>      emit "printf(\"%i\\n\", getINT(L_top));"
>      --emit "printf(\"Stack sz: %i %i\\n\", L_sp - L_base, R_base - R_sp);"
>      emit "gettimeofday(&tv, NULL);"
>      emit "tmp = ((double) tv.tv_sec * 1000000) + (double) tv.tv_usec;"
>      emit "printf(\"GC: %f%%\\n\", 100 * (gc_time / (tmp - prog_start)));"
>      emit "return;"
...
>     , "//printf(\"GC invoked (%i);\\n\", hp - fromSpace);"
>     , "gettimeofday(&tv, NULL);"
>     , "gc_start = ((double) tv.tv_sec * 1000000) + (double) tv.tv_usec;"
>     , "scan = toSpace;"
>     , "free = toSpace;"
