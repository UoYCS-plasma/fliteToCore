...
>      emit ("EVAL_APP(LABEL_" ++ show label ++ ");")

> evalAppMacro :: Compile ()
> evalAppMacro =
>   do emit "#define EVAL_APP(label) \\"
>      let beginLabel  =  "BEGIN_##label"
>      let retLabel    =  "RETURN_##label"
>      block
>        [ beginLabel ++ ":\\"
>        , "if (isFUN(app[1])) {\\"
>        , "  L_PUSH(makePTR(app));\\"
...
Evaluate an atom that is known to reduce to a construction.

> evalMacro :: Compile ()
> evalMacro = 
>   do block
>        [ "#define EVAL(label) \\"
>        , "if (isPTR(atom)) { app = getPTR(atom); \\"
>        ] 
>      emit ("EVAL_APP(label) }\\")
>      block
>        [ "else { L_PUSH(atom); res = getINT(atom); }"
>        ]

> eval :: Compile ()
> eval =
>   do label <- liftFresh freshInt
>      emit ("EVAL(LABEL_" ++ show label ++ ");")

> tailEvalAppMacro :: Compile ()
> tailEvalAppMacro =
>   do emit "#define TAIL_EVAL_APP(label) \\"
>      let beginLabel  =  "BEGIN_##label"
>      block
>        [ beginLabel ++ ":\\"
>        , "if (isFUN(app[1])) {\\"
...
> tailEvalMacro :: Compile ()
> tailEvalMacro = 
>   do block
>        [ "#define TAIL_EVAL(label) \\"
>        , "if (isPTR(atom)) { app = getPTR(atom); \\"
>        ] 
>      emit ("TAIL_EVAL_APP(label) }\\")
>      block
>        [ "else { L_PUSH(atom); res = getINT(atom); }"
>        ]
...
> tailEval =
>   do label <- liftFresh freshInt
>      emit ("TAIL_EVAL(LABEL_" ++ show label ++ ");")
...
>        , "res = " ++ show i ++ ";"
>        ]
>      block [ "goto *(R_top_returnAddress);" | tail ]
...
>        False -> eval
>        True  ->
>          do tailEval -- SOMETIMES INTRODUCES LOOP
>             emit "goto *(R_top_returnAddress);"
